...
function header(context: ReportContext): string {
  ...
  return [
    ...
    '<p class="run-line">' +
      ...
      `<span class="meta-item">stratigraph v${escapeText(TOOL_VERSION)}</span>` +
      '</p>',
    '</header>',
  ].join('\n');
}
...
function cover(data: ReportData, context: ReportContext): string {
  ...
  return [
    ...
    row('Tool', `stratigraph ${TOOL_VERSION}`),
    '</dl>',
...
function footer(run: RunSummary): string {
  return [
    '<footer>',
    `<p>Facts extracted by stratigraph ${escapeText(run.toolVersion)}, run ${run.runId}; ` +
      `report rendered by stratigraph ${escapeText(TOOL_VERSION)}. Regenerating from the ` +
      'same run with the same version produces the same bytes.</p>',
    '</footer>',
  ].join('\n');
}
